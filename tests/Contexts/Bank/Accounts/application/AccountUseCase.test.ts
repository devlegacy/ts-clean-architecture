...
describe(`Bank use case`, () => {
  describe('Bank TestAdapter', () => {
    ...
    beforeAll(() => {
      testAdapter = new AccountUseCase(new InMemoryAccountRepository(), new EURRatioService())
    })
    ...
    it('should reject deposit other currency amount', async () => {
      ...
      }).rejects.toThrow(new InvalidArgumentException('Incompatible currency'))
    })
    ...
    it('should reject withdraw more than balance', async () => {
      ...
      }).rejects.toThrow(new InvalidArgumentException('Insufficient funds'))
    })
