...
const getRouteMethodMetadata = (method: () => unknown) => {
  const routePath: RequestMappingMetadata[typeof PATH_METADATA] = Reflect.getMetadata(
    PATH_METADATA,
    method,
  )
  ...
  const schema: { schema: FastifySchema, code: number } = Reflect.getMetadata(
    SCHEMA_METADATA,
    method,
  ) || {
    schema: {},
...
const getKeyParam = (params: Record<string, RouteParamMetadata>): [number, number, string][] =>
  Object.keys(params).map((keyParam: string) => [
    ...(keyParam.split(':').map((value) => parseInt(
      value,
      10,
    )) as [number, number]),
    keyParam,
...
const getParams = (
  ...
  for (const keyParam of keyParams) {
    ...
    const {
      data, pipes,
    } = params[`${key}`] || DEFAULT_METADATA

    const routeParamValue = ParamExtractors[+paramtype]?.extract(
      req,
      res,
      data,
      pipes,
...
const clusterServer = (
  ...
  {
    method, schema, url, httpCode, params, instance, methodName,
  }: Route,
  isProduction = false,
) => {
...
const routeBuilder = (adapterInstance: FastifyAdapter, controller: Class<unknown>, props: RouteRegisterProps) => {
  const {
    instance, instanceConstructor, instancePrototype, classMethodNames, controllerPath,
  ...
  for (const methodName of classMethodNames) {
    ...
    const {
      routePath, requestMethod, httpCode, schema,
    } = getRouteMethodMetadata(method)
    ...
    if (params) {
      ...
      buildSchemaWithParams(
        params,
        schema,
        requestMethod,
        args,
        adapterInstance.validations,
      )
    }
